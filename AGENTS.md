...
- Agents are defined as Markdown + YAML frontmatter files under `.pi/agents/` (this project's custom directory): name, mark, tagline, description, suggestions, and the body as the system prompt. Adding a file adds an Agent. Agents run with a host-controlled pilot tool allowlist (`read`/`grep`/`find`/`ls`/`bash`); never let an Agent file declare or expand tools.
- Tool calls are guarded by the `pi-permission-system` extension (HITL approvals). It ships TS-only sources with a broken `getApiProvider` import, fixed by `patches/pi-permission-system@0.8.0.patch` (pnpm `patchedDependencies`), and is bundled at build time to `packages/pi-agent/dist/permission-system.mjs` by `scripts/build-extensions.mjs` — production code only dynamic-imports that artifact. Policy lives in `.pi/agent/pi-permissions.jsonc`; headless ask decisions are forwarded as files under `.pi/permission-forwarding/` (runtime state, gitignored), bridged into SQLite (`approvals` table) by `packages/pi-agent/src/approvals.ts` and decided through the `/api/v1/approvals` endpoints.
- SubAgent delegation comes from the `pi-subagents` extension (nicobailon), loaded by `packages/pi-agent/src/subagents.ts` via jiti from the real install path — do NOT bundle it (it locates runtime assets like child-process extensions and built-in agents relative to `import.meta.url`; bundling breaks those paths). The host must add `subagent` to the session tool allowlist and allow it in `.pi/agent/pi-permissions.jsonc` (delegation is orchestration only; the spawned child pi process gets its own approvals through the inherited forwarding env). `createAgentSession()` never emits `session_start` by itself (only the CLI modes call `session.bindExtensions()`), so `createPiAgentSession` must call `await session.bindExtensions({})` after session creation — without it pi-permission-system never rebuilds its PermissionManager with `ctx.cwd` and the project policy file is silently ignored.
