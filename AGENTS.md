...
- Tool calls are guarded by the `pi-permission-system` extension (HITL approvals). It ships TS-only sources with a broken `getApiProvider` import, fixed by `patches/pi-permission-system@0.8.0.patch` (pnpm `patchedDependencies`), and is bundled at build time to `packages/pi-agent/dist/permission-system.mjs` by `scripts/build-permission-ext.mjs` — production code only dynamic-imports that artifact. Policy lives in `.pi/agent/pi-permissions.jsonc`; headless ask decisions are forwarded as files under `.pi/permission-forwarding/` (runtime state, gitignored), bridged into SQLite (`approvals` table) by `packages/pi-agent/src/approvals.ts` and decided through the `/api/v1/approvals` endpoints.
