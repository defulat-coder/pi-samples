...
- This repository is a TypeScript monorepo for a Web-triggered, multi-business-agent Pi Coding Agent playground.
...
- Official Pi built-ins include write-capable tools. This project uses host-selected, read-only capability profiles: the knowledge Agent exposes `read` plus `search_knowledge`; the business-data Agent exposes `read` plus `query_business_data`. Keep every profile read-only and let Pi choose when to call its available tools.
- Bind every persisted Session to one immutable `agentId`; reject attempts to reuse that Session through another Agent profile, and key in-process runtimes by `agentId + sessionId`.
...
- Business rich UI transports one authoritative `BusinessAnalyticalResult` plus a vendor-neutral `BusinessPresentationPlan`. The Web adapter deterministically creates a guardrailed json-render spec from field references; presentation data must never become a second source of metric values. Preserve Markdown as the explanation and fallback layer.
