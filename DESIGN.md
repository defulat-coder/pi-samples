...
- 收件箱行 44px：AgentChip + agent 名 + 状态胶囊（仅需处理行）+ 标题/预览 + 时间戳；未读 = 600 字重无预览，已读 = 常规字重 + ` — ` 预览；hover 行尾浮现 选择/已读切换/删除 三个行内按钮（选中行常驻）
- 双击行进入左右分栏：列表收窄为 352px 左栏（隐藏 agent 名保 chip），右栏滑入（x 24px → 0，0.2s）线程详情——头部 AgentChip + agent 名 + 会话标题 + 「在聊天中打开」+ 关闭 X；需处理会话在消息上方放警示琥珀提示条
- 页头为标题左置 + 320px 搜索框绝对居中；分段 tabs 复用 segmented 页签规格，「需处理」页签带未读计数
