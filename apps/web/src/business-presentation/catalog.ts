...
export const businessPresentationCatalog = defineCatalog(schema, {
  components: {
    ...
    LineChart: { props: z.object({ title: z.string(), unit: z.string(), labels: z.array(z.string()), series: z.array(z.object({ name: z.string(), values: z.array(z.number().nullable()) })) }), description: '时间维度单序列或多序列趋势图' },
