...
本地 SQLite 提供 365 天、52,560 行日粒度电商经营演示数据，截止 `2026-08-18`，覆盖 6 个区域、4 个渠道和 6 个品类。20 个经营场景由 Codex CLI 使用 `gpt-5.6-luna` 以 20 路并行生成，再由版本化本地生成器扩展为完整数据；金额以整数分存储，查询结果再转换为元。

可通过 `pnpm business:data:generate:luna` 重新生成场景快照。生成器使用严格 JSON Schema、只读 ephemeral Codex CLI 会话、默认 20 路并行和单分片失败重试；20 份结果全部通过校验后才原子替换 `business-scenarios.json`。
